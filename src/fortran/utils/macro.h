 ...
 *          procedures. The macros handle compiler-specific differences.
 *****************************************************************************/

#define ERROR_LOCATION __FILE__, __LINE__ /*< Macro for file and line location */

#ifdef __GFORTRAN__
/** @brief Assert that a logical expression is true (gfortran version) */
#define CHECK_TRUE(EXPR, FORMAT) call ASSERT_TRUE(EXPR, FORMAT, ERROR_LOCATION)
/** @brief Assert that two integer values are equal (gfortran version) */
#define CHECK_EQUAL(VALUE, EXPECTED, FORMAT) call ASSERT_EQUAL(VALUE, EXPECTED, FORMAT, ERROR_LOCATION)
/** @brief Assert that two floating-point values are equal within tolerance (gfortran version) */
